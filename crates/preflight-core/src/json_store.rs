...
mod tests {
    ...
    fn create_review_with_store(store: &JsonFileStore) -> Review {
        store
    ...
    fn test_create_and_get_review() {
        ...
        let fetched = store.get_review(review.id).unwrap();
    ...
    fn test_get_review_not_found() {
        let (store, _dir) = test_store();
        let result = store.get_review(Uuid::new_v4());
    ...
    fn test_update_review_status() {
        ...
        store
            ...
            .unwrap();
        let updated = store.get_review(review.id).unwrap();
    ...
    fn test_create_and_get_thread() {
        let (store, _dir) = test_store();
        let review = create_review_with_store(&store);
        let thread = store
    ...
    #[test]
    fn test_create_thread_review_not_found() {
        let (store, _dir) = test_store();
        let result = store.create_thread(CreateThreadInput {
    ...
    #[test]
    fn test_get_threads_filtered_by_file() {
        let (store, _dir) = test_store();
        let review = create_review_with_store(&store);
        store
            .create_thread(CreateThreadInput {
        ...
        store
            ...
            .unwrap();
        let all = store.get_threads(review.id, None).unwrap();
        assert_eq!(all.len(), 2);
        let filtered = store.get_threads(review.id, Some("src/a.rs")).unwrap();
    ...
    fn test_update_thread_status() {
        ...
        store
            ...
            .unwrap();
        let threads = store.get_threads(review.id, None).unwrap();
        assert_eq!(threads[0].status, ThreadStatus::Resolved);
    }

    #[test]
    fn test_add_comment_to_thread() {
        let (store, _dir) = test_store();
        let review = create_review_with_store(&store);
        let thread = store
            .create_thread(CreateThreadInput {
    ...
    #[test]
    fn test_add_comment_thread_not_found() {
        let (store, _dir) = test_store();
        let result = store.add_comment(AddCommentInput {
    ...
    fn test_threads_persist() {
        ...
        {
            let store = JsonFileStore::new(&path).unwrap();
            let threads = store.get_threads(review_id, None).unwrap();
